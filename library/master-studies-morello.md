---
title: Master Studies (Morello) — Exercise Catalog
type: catalog
permalink: music-kb/library/master-studies-morello
book_slug: master-studies-morello
instrument: drums
tags:
- catalog
- drums
- morello
- master-studies
---

# Master Studies — Exercise Catalog
*Joe Morello*

Source slug: `master-studies-morello`. 97 pages.

## Exercises Worked On

### pp.7–10 — Accent Studies
- **Chapter:** Accent Studies (pp.7–19)
- **Focus:** accent layering over single strokes — full stroke vs. ghost note contrast
- **Tags:** hands, accents, dynamics, single-strokes
- **Status:** in-rotation
- **Last worked:** 2026-06-14
- **BPM History:**
  - 2026-06-14: first session — fun. No BPM logged, focused on accent/ghost contrast.
- **Notes:** Fresh chapter, immediately enjoyable. Exaggerate contrast — accents as punch, ghosts near-silent.

### p.44 — Table of Time
- **Chapter:** Control Studies (pp.40–65)
- **Focus:** subdivision changes, internal clock
- **Tags:** subdivisions, hands, control, internal-clock
- **Status:** in-rotation
- **Current ceiling:** 53 BPM halftime
- **Bottlenecks:** Rushes on subdivision transitions
- **Notes:** Earlier routines cited p.43 — Table of Time is actually p.44. p.43 is not the exercise.

### p.54 — Stone Killer
- **Chapter:** Control Studies (pp.40–65)
- **Focus:** dynamic control, single strokes
- **Tags:** dynamics, control, single-strokes, kit-incorporation
- **Status:** attempted
- **Last worked:** 2026-05-12
- **Bottlenecks:** Dynamics don't transfer well from pad to kit
- **Notes:** Try with different kit voicings to build dynamic awareness on full kit.

### p.71 — Groups of Four with Fill-Ins
- **Chapter:** Fill-In Studies (pp.66–76)
- **Focus:** double bass evenness, left foot stability
- **Tags:** double-bass, left-foot, single-strokes, fill-ins
- **Status:** bottlenecked
- **Current ceiling:** 72 BPM stable
- **Last worked:** 2026-05-04
- **BPM History:**
  - 2026-05-02: 70 BPM baseline
  - 2026-05-04: 72 BPM stable; left foot flutters above
- **Bottlenecks:** Left foot speeds up at higher tempos, tip-toeing/fluttering. Right foot more stable. Target evenness, not speed.

## Candidates for Rotation
- p.66 — Groups of Three with Fill-Ins: sister exercise to p.71, different odd grouping
- pp.7–19 — Accent Studies: in-rotation as of 2026-06-14 — fun, first session
- pp.20–29 — Buzz Roll Studies: builds press-roll vocabulary
- pp.30–39 — Stroke Combination Studies: stickings for kit application
- pp.82–93 — Flam Studies: after Stick Control flams
